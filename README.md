...
*Tips: 如果在提交题包中的题目的代码前没有开始过这个题包，可能会无法记录提交结果。开始题包方法参照：[开始题包](#开始题包)。提交之前请确保 `pid`, `eid`正确。可以获取题面以验证，方法参照：[查看题目内容](#查看题目内容)。另外建议提交之前检查一遍 `solutioncode`. 查询方法参照：[查询变量](#查询变量)。*
...
## 变量相关
用户能查询、修改和重置的变量全部位于`requests_var`.
*Tips: 同时位于 `requests_var` 和 `net_var` 的变量不能修改.*

### 查询变量
`show [VAR]`.  
查询变量（别）名VAR的变量; 若没有指定变量名, 则返回`requests_var`中的所有变量. 返回类型为*json*.
