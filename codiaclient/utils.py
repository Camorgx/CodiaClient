...
class AliasesDict(dict):
    ...
    def __getitem__(self, attr):
        if attr in self.aliaseslist:
            return super().__getitem__(self.aliaseslist[attr])
        else:
            return super().__getitem__(attr)

    def __setitem__(self, key, value):
        if key in self.aliaseslist:
            return super().__setitem__(self.aliaseslist[key], value)
        else:
...
def cookie_encrypt(cookie, passwd):
    return str(base64.encodebytes(AES.new(add_to_16(passwd), AES.MODE_ECB).encrypt(add_to_16(cookie))),
               encoding='utf-8')


def cookie_decrypt(_cookie, passwd):
    return str(AES.new(add_to_16(passwd), AES.MODE_ECB).decrypt(base64.decodebytes(_cookie.encode(encoding='utf-8'))),
